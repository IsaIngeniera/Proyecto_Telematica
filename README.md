...
.
├── cliente/
│   ├── main.py                    # Aplicación cliente con interfaz gráfica Tkinter[cite: 3]
│   ├── protocolo_cliente.py       # Serializador y parser de tramas TCP para la GUI[cite: 3]
│   └── servicio_web.py            # Servidor HTTP Flask (puerto 8080) y vista web[cite: 3]
├── nodos/
│   ├── nodo.py                    # Emulador de nodo sensor individual con manejo de sockets[cite: 3]
│   ├── lanzar_nodos.py            # Orquestador multi-proceso para pruebas de concurrencia[cite: 3]
│   ├── protocolo.py               # Lógica común compartida para formateo de tramas[cite: 3]
│   └── test_protocolo.py          # Pruebas unitarias de construcción y validación de mensajes[cite: 3]
├── servidor/
│   ├── Dockerfile                 # Receta de compilación nativa en C sobre debian:bookworm-slim[cite: 3]
│   ├── docker-compose.yml         # Orquestación del servicio y forward de puertos[cite: 3]
│   ├── Makefile                   # Reglas de compilación y limpieza con gcc[cite: 3]
│   ├── server.c                   # Punto de entrada, bucle select(), sockets TCP/UDP y señales[cite: 3]
│   ├── protocolo.c / .h           # Procesamiento de líneas de texto y respuestas del protocolo[cite: 3]
│   └── nodos.c / .h               # Estructuras de datos dinámicas, estados y reglas de alerta[cite: 3]
├── capturas/
│   └── captura_telematica.pcapng  # Archivo de trazas de red capturado en Wireshark[cite: 2, 3]
└── README.md                      # Documentación técnica del proyecto[cite: 3]
...
* **Alertas críticas detectadas:** 7 alertas generadas de forma determinista ante la anomalía térmica de $45.00^{\circ}\text{C}$ programada en `NODE01`.
